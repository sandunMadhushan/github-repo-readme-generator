...
export function RepoInput({ onRepoAnalyzed, isLoading, setIsLoading }: RepoInputProps) {
  ...
  const analyzeRepository = async () => {
    ...
    try {
      ...
      const features = generateFeatures(repoData, contents, dependencies, devDependencies)
  ...
  const generateFeatures = (
    ...
    if (deps.typescript || devDeps.typescript) features.push('TypeScript Support')
    
    // Testing
    if (devDeps.jest || devDeps.vitest || devDeps.mocha) features.push('Unit Testing')
    ...
    // Documentation
    if (contents.some(file => file.name.includes('docs'))) features.push('Documentation')
    
    // Docker
    if (contents.some(file => file.name === 'Dockerfile')) features.push('Docker Support')
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-8 border border-gray-200">
      <div className="text-center mb-8">
        ...
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Analyze Your Repository</h2>
        <p className="text-gray-600">
          Enter your GitHub repository URL to get started with AI-powered README generation
        </p>
      </div>
      ...
      <div className="mt-8 text-center text-sm text-gray-500">
        <p>
          We'll analyze your repository structure, dependencies, and generate a comprehensive README
        </p>
      </div>
    </div>
  )
